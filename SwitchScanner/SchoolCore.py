...
def loop_finder_9900(schoolcores_9900, searchstartdate):
    ...
    :param schoolcores_9900: school core tuple
    ...
    device, site_name = schoolcores_9900
...
def wan_health_9900(schoolcores_9900, searchstartdate):
    ...
    device, site_name = schoolcores_9900  # Iterate through devices
    # for each device reset data to blank string
    data = ''
    for command in commands:  # iterate through commands
        # store command output to sshdata variable
...
def health_audit_9900(schoolcores_9900):
    ...
    if data:
        ...
        if re.findall(r"down", data, flags=re.IGNORECASE):
            # Add message to issue log array
            issue_log.append('Down state discovered.  Check all modules and fabrics on this device for down status.\n')
    if issue_log:
    ...
    return switchcount, wan_isc, wan_west, cfm_a, cfm_b, watts2400, watts3600, cmm_a_up, cmm_b_up, cmm_a_secondary,\
           cmm_b_secondary, devices_with_issues
...
def parse_health_audit_9900(switchcount, wan_isc, wan_west, cfm_a, cfm_b, watts2400, watts3600,
    ...
    if devices_with_issues:
        ...
        text += 'No issues were discovered in the 9900 General Health Audit.\n'
    return text
